...
def get_unsuccessful_patch_pull_request_body(source_branch, source_sha, patch_failures):
    """
    Returns the body to post in the pull request description when the auto-patch was unsuccessful.

    Inputs:
    - source_branch: the source branch to apply patches from
    ...
    message = [
        "",
        f"We encountered some errors when trying to upgrade c3standards from `{source_sha}` to the latest `{source_branch}` in your repository.",
        "- Rejected patches",
        f"{patch_failures}",
        "",
        "",
        ...
        "   - If there are fewer `.rej` files than the list shown above, it means that the upgrader had trouble deleting files that exist in your repo but not in c3standards",
...
def get_successful_patch_pull_request_body(source_branch, source_sha):
    """
    ...
    message = [
        "",
        f"We successfully upgraded c3standards from `{source_sha}` to the latest `{source_branch}` in your repository.",
...
def perform_patch_upgrade(
    ...
    if always_create_pr:
        ...
        pull_request_body = get_successful_patch_pull_request_body(source_branch, current_c3standards_sha)
    ...
    sh(
        [
            ...
            "commit",
            "-m",
            f"[NO UPSTREAM MERGE] Automated chore: Upgraded c3standards to latest master ({latest_c3standards_sha})",
            "--no-verify",
            "--no-edit",
        ]
    )
...
def perform_partial_patch_upgrade(
    ...
    pull_request_body = get_unsuccessful_patch_pull_request_body(source_branch, current_c3standards_sha, patch_failures)
    generate_pull_request(repo, target_branch, latest_c3standards_sha, pull_request_title, pull_request_body)
...
def check_can_auto_apply_patch(upgrade_patch_filename):
    ...
    print("Can auto-apply patch!")
    return True, None


def upgrade_c3standards(repo, source_branch, target_branch, always_create_pr, workspace):
    """
    ...
    - source_branch: the source branch to apply the patches from
    ...
    fetch_checkout_pull(f"{source_branch}", f"{workspace}/c3standards")

    rcfile_contents = read_rcfile(".c3standardsrc")
    current_c3standards_sha = rcfile_contents["sha"] if rcfile_contents else None
    latest_c3standards_sha = sh_cwd(f"git rev-parse {source_branch}", f"{workspace}/c3standards")
    ...
    if not current_c3standards_sha:
        current_c3standards_sha_str = sh_cwd(
            f"git rev-list --max-parents=0 {source_branch}", f"{workspace}/c3standards", False
        )
...
def main():
    ...
    upgrade_c3standards(
        github_context["event"]["repository"]["name"], source_branch, target_branch, always_create_pr, workspace
    )
